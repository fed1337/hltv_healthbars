...
pfnUserMsgHook HookUserMsg(char* szMsgName, void* pfnUserMessage)
{
	...
	while (pList)
	{
		if (!strcmp(pList->pszFuncName, szMsgName))
		{
			pfnUserMsgHook ret = pList->pfnUserMessage;
			pList->pfnUserMessage = (pfnUserMsgHook)pfnUserMessage;
			return ret;
		}
		pList = pList->pNext;
	}
...
int TeamScore (const char* szMsgName, int iSize, pfnUserMsgHook *pBuf)
{
...
int RoundTime (const char* szMsgName, int iSize, pfnUserMsgHook *pBuf)
{
	BEGIN_READ(pBuf, iSize);
	int time = READ_SHORT();
	...
	g_HUD_Vars.bBombPlanted = false; // the Bomb_Planted TextMsg is only ever sent after RoundTime messages, no need to reset it elsewhere
...
int TextMsg (const char* szMsgName, int iSize, pfnUserMsgHook *pBuf)
{
	BEGIN_READ(pBuf, iSize);
	int destinationType = READ_BYTE();
	char* message = READ_STRING();
	if (!strcmp(message, "#Bomb_Planted"))
	{
		g_HUD_Vars.bBombPlanted = true;
		g_HUD_Vars.iRoundTime = cvars->getConVarFloat("c4timer");
	}
...
int ScoreInfo(const char* szMsgName, int iSize, pfnUserMsgHook *pBuf)
{
	BEGIN_READ(pBuf, iSize);
...
int DeathMsg (const char* szMsgName, int iSize, pfnUserMsgHook *pBuf)
{
	BEGIN_READ(pBuf, iSize);
