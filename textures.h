...
#define DELETE_TEXTURE(texture) unsigned int oldTextureId_#texture = texture.texID; glDeleteTextures(1, &oldTextureId_#texture);
#define DBG_PRINT_TEXTURE(texture) g_oEngine.Con_Printf("%s (id %u): %dx%d\r\n", #texture, texture.texID, texture.width, texture.height);