...
*/
auto lodepng_decode_memory(unsigned char** out, unsigned* w, unsigned* h,
                               const unsigned char* in, size_t insize,
                               LodePNGColorType colortype, unsigned bitdepth) -> unsigned;

/*Same as lodepng_decode_memory, but always decodes to 32-bit RGBA raw image*/
auto lodepng_decode32(unsigned char** out, unsigned* w, unsigned* h,
                          const unsigned char* in, size_t insize) -> unsigned;

/*Same as lodepng_decode_memory, but always decodes to 24-bit RGB raw image*/
auto lodepng_decode24(unsigned char** out, unsigned* w, unsigned* h,
                          const unsigned char* in, size_t insize) -> unsigned;
...
auto lodepng_decode_file(unsigned char** out, unsigned* w, unsigned* h,
                             const char* filename,
                             LodePNGColorType colortype, unsigned bitdepth) -> unsigned;
...
NOTE: Wide-character filenames are not supported, you can use an external method
to handle such files and decode in-memory.*/
auto lodepng_decode32_file(unsigned char** out, unsigned* w, unsigned* h,
...
NOTE: Wide-character filenames are not supported, you can use an external method
to handle such files and decode in-memory.*/
auto lodepng_decode24_file(unsigned char** out, unsigned* w, unsigned* h,
                               const char* filename) -> unsigned;
...
Return value: LodePNG error code (0 means no error).
*/
auto lodepng_encode_memory(unsigned char** out, size_t* outsize,
                               const unsigned char* image, unsigned w, unsigned h,
                               LodePNGColorType colortype, unsigned bitdepth) -> unsigned;

/*Same as lodepng_encode_memory, but always encodes from 32-bit RGBA raw image.*/
auto lodepng_encode32(unsigned char** out, size_t* outsize,
                          const unsigned char* image, unsigned w, unsigned h) -> unsigned;

/*Same as lodepng_encode_memory, but always encodes from 24-bit RGB raw image.*/
auto lodepng_encode24(unsigned char** out, size_t* outsize,
                          const unsigned char* image, unsigned w, unsigned h) -> unsigned;
...
auto lodepng_encode_file(const char* filename,
                             const unsigned char* image, unsigned w, unsigned h,
                             LodePNGColorType colortype, unsigned bitdepth) -> unsigned;
...
NOTE: Wide-character filenames are not supported, you can use an external method
to handle such files and encode in-memory.*/
auto lodepng_encode32_file(const char* filename,
                               const unsigned char* image, unsigned w, unsigned h) -> unsigned;
...
NOTE: Wide-character filenames are not supported, you can use an external method
to handle such files and encode in-memory.*/
auto lodepng_encode24_file(const char* filename,
                               const unsigned char* image, unsigned w, unsigned h) -> unsigned;
#endif /*LODEPNG_COMPILE_DISK*/
...
auto decode(std::vector<unsigned char>& out, unsigned& w, unsigned& h,
                const unsigned char* in, size_t insize,
                LodePNGColorType colortype = LCT_RGBA, unsigned bitdepth = 8) -> unsigned;
auto decode(std::vector<unsigned char>& out, unsigned& w, unsigned& h,
...
auto encode(std::vector<unsigned char>& out,
...
to handle such files and decode in-memory.
*/
auto encode(const std::string& filename,
                const unsigned char* in, unsigned w, unsigned h,
                LodePNGColorType colortype = LCT_RGBA, unsigned bitdepth = 8) -> unsigned;
auto encode(const std::string& filename,
                const std::vector<unsigned char>& in, unsigned w, unsigned h,
...
auto lodepng_error_text(unsigned code) -> const char*;
...
auto lodepng_color_mode_copy(LodePNGColorMode* dest, const LodePNGColorMode* source) -> unsigned;
/* Makes a temporary LodePNGColorMode that does not need cleanup (no palette) */
auto lodepng_color_mode_make(LodePNGColorType colortype, unsigned bitdepth) -> LodePNGColorMode;

void lodepng_palette_clear(LodePNGColorMode* info);
/*add 1 color to the palette*/
auto lodepng_palette_add(LodePNGColorMode* info,
                             unsigned char r, unsigned char g, unsigned char b, unsigned char a) -> unsigned;

/*get the total amount of bits per pixel, based on colortype and bitdepth in the struct*/
auto lodepng_get_bpp(const LodePNGColorMode* info) -> unsigned;
/*get the amount of color channels used, based on colortype in the struct.
If a palette is used, it counts as 1 channel.*/
auto lodepng_get_channels(const LodePNGColorMode* info) -> unsigned;
/*is it a grayscale type? (only colortype 0 or 4)*/
auto lodepng_is_greyscale_type(const LodePNGColorMode* info) -> unsigned;
/*has it got an alpha channel? (only colortype 2 or 6)*/
auto lodepng_is_alpha_type(const LodePNGColorMode* info) -> unsigned;
/*has it got a palette? (only colortype 3)*/
auto lodepng_is_palette_type(const LodePNGColorMode* info) -> unsigned;
/*only returns true if there is a palette and there is a value in the palette with alpha < 255.
Loops through the palette to check this.*/
auto lodepng_has_palette_alpha(const LodePNGColorMode* info) -> unsigned;
/*
...
*/
auto lodepng_can_have_alpha(const LodePNGColorMode* info) -> unsigned;
/*Returns the byte size of a raw image buffer with given width, height and color mode*/
auto lodepng_get_raw_size(unsigned w, unsigned h, const LodePNGColorMode* color) -> size_t;
...
/*return value is error code (0 means no error)*/
auto lodepng_info_copy(LodePNGInfo* dest, const LodePNGInfo* source) -> unsigned;

#ifdef LODEPNG_COMPILE_ANCILLARY_CHUNKS
auto lodepng_add_text(LodePNGInfo* info, const char* key, const char* str) -> unsigned; /*push back both texts at once*/
void lodepng_clear_text(LodePNGInfo* info); /*use this to clear the texts again after you filled them in*/

auto lodepng_add_itext(LodePNGInfo* info, const char* key, const char* langtag,
                           const char* transkey, const char* str) -> unsigned; /*push back the 4 texts of 1 chunk at once*/
void lodepng_clear_itext(LodePNGInfo* info); /*use this to clear the itexts again after you filled them in*/

/*replaces if exists*/
auto lodepng_set_icc(LodePNGInfo* info, const char* name, const unsigned char* profile, unsigned profile_size) -> unsigned;
void lodepng_clear_icc(LodePNGInfo* info); /*use this to clear the profile again after you filled it in*/

/*replaces if exists*/
auto lodepng_set_exif(LodePNGInfo* info, const unsigned char* exif, unsigned exif_size) -> unsigned;
...
*/
auto lodepng_convert(unsigned char* out, const unsigned char* in,
...
auto lodepng_compute_color_stats(LodePNGColorStats* stats,
                                     const unsigned char* image, unsigned w, unsigned h,
                                     const LodePNGColorMode* mode_in) -> unsigned;
...
/*return value is error code (0 means no error)*/
auto lodepng_state_copy(LodePNGState* dest, const LodePNGState* source) -> unsigned;
...
*/
auto lodepng_decode(unsigned char** out, unsigned* w, unsigned* h,
                        LodePNGState* state,
                        const unsigned char* in, size_t insize) -> unsigned;
...
*/
auto lodepng_inspect(unsigned* w, unsigned* h,
                         LodePNGState* state,
                         const unsigned char* in, size_t insize) -> unsigned;
#endif /*LODEPNG_COMPILE_DECODER*/
...
*/
auto lodepng_inspect_chunk(LodePNGState* state, size_t pos,
                               const unsigned char* in, size_t insize) -> unsigned;

#ifdef LODEPNG_COMPILE_ENCODER
/*This function allocates the out buffer with standard malloc and stores the size in *outsize.*/
auto lodepng_encode(unsigned char** out, size_t* outsize,
                        const unsigned char* image, unsigned w, unsigned h,
                        LodePNGState* state) -> unsigned;
#endif /*LODEPNG_COMPILE_ENCODER*/
...
*/
auto lodepng_chunk_length(const unsigned char* chunk) -> unsigned;

/*puts the 4-byte type in null terminated string*/
void lodepng_chunk_type(char type[5], const unsigned char* chunk);

/*check if the type is the given type*/
auto lodepng_chunk_type_equals(const unsigned char* chunk, const char* type) -> unsigned char;

/*0: it's one of the critical chunk types, 1: it's an ancillary chunk (see PNG standard)*/
auto lodepng_chunk_ancillary(const unsigned char* chunk) -> unsigned char;

/*0: public, 1: private (see PNG standard)*/
auto lodepng_chunk_private(const unsigned char* chunk) -> unsigned char;

/*0: the chunk is unsafe to copy, 1: the chunk is safe to copy (see PNG standard)*/
auto lodepng_chunk_safetocopy(const unsigned char* chunk) -> unsigned char;

/*get pointer to the data of the chunk, where the input points to the header of the chunk*/
auto lodepng_chunk_data(unsigned char* chunk) -> unsigned char*;
auto lodepng_chunk_data_const(const unsigned char* chunk) -> const unsigned char*;

/*returns 0 if the crc is correct, 1 if it's incorrect (0 for OK as usual!)*/
auto lodepng_chunk_check_crc(const unsigned char* chunk) -> unsigned;
...
*/
auto lodepng_chunk_next(unsigned char* chunk, unsigned char* end) -> unsigned char*;
auto lodepng_chunk_next_const(const unsigned char* chunk, const unsigned char* end) -> const unsigned char*;

/*Finds the first chunk with the given type in the range [chunk, end), or returns NULL if not found.*/
auto lodepng_chunk_find(unsigned char* chunk, unsigned char* end, const char type[5]) -> unsigned char*;
auto lodepng_chunk_find_const(const unsigned char* chunk, const unsigned char* end, const char type[5]) -> const unsigned char*;
...
*/
auto lodepng_chunk_append(unsigned char** out, size_t* outsize, const unsigned char* chunk) -> unsigned;
...
Returns error code (0 if it went ok)
*/
auto lodepng_chunk_create(unsigned char** out, size_t* outsize, size_t length,
                              const char* type, const unsigned char* data) -> unsigned;


/*Calculate CRC32 of buffer*/
auto lodepng_crc32(const unsigned char* buf, size_t len) -> unsigned;
#endif /*LODEPNG_COMPILE_PNG*/
...
auto lodepng_inflate(unsigned char** out, size_t* outsize,
                         const unsigned char* in, size_t insize,
                         const LodePNGDecompressSettings* settings) -> unsigned;
...
*/
auto lodepng_zlib_decompress(unsigned char** out, size_t* outsize,
                                 const unsigned char* in, size_t insize,
                                 const LodePNGDecompressSettings* settings) -> unsigned;
#endif /*LODEPNG_COMPILE_DECODER*/
...
buffer and *outsize its size in bytes. out must be freed by user after usage.
*/
auto lodepng_zlib_compress(unsigned char** out, size_t* outsize,
                               const unsigned char* in, size_t insize,
...
*/
auto lodepng_huffman_code_lengths(unsigned* lengths, const unsigned* frequencies,
                                      size_t numcodes, unsigned maxbitlen) -> unsigned;

/*Compress a buffer with deflate. See RFC 1951. Out buffer must be freed after use.*/
auto lodepng_deflate(unsigned char** out, size_t* outsize,
                         const unsigned char* in, size_t insize,
                         const LodePNGCompressSettings* settings) -> unsigned;
...
to handle such files and decode in-memory.
*/
auto lodepng_load_file(unsigned char** out, size_t* outsize, const char* filename) -> unsigned;
...
to handle such files and encode in-memory
*/
auto lodepng_save_file(const unsigned char* buffer, size_t buffersize, const char* filename) -> unsigned;
#endif /*LODEPNG_COMPILE_DISK*/
...
auto decode(std::vector<unsigned char>& out, unsigned& w, unsigned& h,
                State& state,
                const std::vector<unsigned char>& in) -> unsigned;
#endif /*LODEPNG_COMPILE_DECODER*/
...
to handle such files and decode in-memory
*/
auto load_file(std::vector<unsigned char>& buffer, const std::string& filename) -> unsigned;
...
to handle such files and encode in-memory
*/
auto save_file(const std::vector<unsigned char>& buffer, const std::string& filename) -> unsigned;
#endif /* LODEPNG_COMPILE_DISK */
#endif /* LODEPNG_COMPILE_PNG */
...
auto decompress(std::vector<unsigned char>& out, const unsigned char* in, size_t insize,
                    const LodePNGDecompressSettings& settings = lodepng_default_decompress_settings) -> unsigned;

/* Zlib-decompress an std::vector */
auto decompress(std::vector<unsigned char>& out, const std::vector<unsigned char>& in,
                    const LodePNGDecompressSettings& settings = lodepng_default_decompress_settings) -> unsigned;
#endif /* LODEPNG_COMPILE_DECODER */

#ifdef LODEPNG_COMPILE_ENCODER
/* Zlib-compress an unsigned char buffer */
auto compress(std::vector<unsigned char>& out, const unsigned char* in, size_t insize,
                  const LodePNGCompressSettings& settings = lodepng_default_compress_settings) -> unsigned;

/* Zlib-compress an std::vector */
auto compress(std::vector<unsigned char>& out, const std::vector<unsigned char>& in,
                  const LodePNGCompressSettings& settings = lodepng_default_compress_settings) -> unsigned;
#endif /* LODEPNG_COMPILE_ENCODER */
#endif /* LODEPNG_COMPILE_ZLIB */
} /* namespace lodepng */
