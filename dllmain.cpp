...
sprite_data_s* g_Sprites = NULL;
char** g_pszPlayerNames = NULL;
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		CreateThread(0, 0, (LPTHREAD_START_ROUTINE)InitializeHack, 0, 0, 0);
		break;
...
void InitializeHack()
{
	...
	char szPath[256];
	int len = (int)GetModuleFileNameA(NULL, szPath, sizeof(szPath));
	szPath[len-6] = 0; // remove "hl.exe"
	sprintf(g_szPluginPath, "%sdj\\", szPath);
