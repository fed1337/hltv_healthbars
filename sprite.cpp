...
void LoadSprites()
{
	...
	while (!feof(fp) && i < lines) 
	{
		...
		fscanf(fp,"%d %d %d %d", 
			&g_Sprites[i].rc.left, &g_Sprites[i].rc.right, 
			&g_Sprites[i].rc.top, &g_Sprites[i].rc.bottom);
...
int GetSpriteIndex(char* szSpriteName)
{
	for (int i = 0; i < g_iSpriteCount; i++)
	{
		if (g_Sprites[i].iRes == 640 && strstr(g_Sprites[i].szSpriteName, szSpriteName))
